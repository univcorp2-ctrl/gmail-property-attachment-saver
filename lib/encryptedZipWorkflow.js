...
export function evaluateRuntimeReadiness(input = {}) {
  ...
  if (input.gmailZipBytesReadable && input.passwordMailReadable && input.extractorAvailable && input.driveWritable) {
    routes.push('direct');
  }
  ...
  if (
    input.gasDeployed &&
    input.gasAuthorized &&
    input.windowsExecutorReachable &&
    input.bridgeAuthAvailable &&
    input.extractorAvailable &&
    input.driveWritable
  ) {
    routes.push('gas_windows');
  }

  if (input.localZipPresent && input.passwordMailReadable && input.extractorAvailable && input.driveWritable) {
    routes.push('local_existing_zip');
  }

  const blockers = [];
  if (!input.driveWritable) blockers.push('DRIVE_NOT_WRITABLE');
  if (!input.extractorAvailable) blockers.push('EXTRACTOR_NOT_AVAILABLE');

  const canAcquireZip = Boolean(
    input.gmailZipBytesReadable ||
    (input.gasDeployed && input.gasAuthorized) ||
    input.localZipPresent
  );
  if (!canAcquireZip) blockers.push('ZIP_BYTES_ROUTE_UNAVAILABLE');

  const canReadPasswordMail = Boolean(input.passwordMailReadable || input.gasAuthorized);
  if (!canReadPasswordMail) blockers.push('PASSWORD_MAIL_ROUTE_UNAVAILABLE');

  const ready = routes.length > 0;
  if (!ready && input.gasDeployed && input.gasAuthorized && !input.windowsExecutorReachable) {
    blockers.push('WINDOWS_EXECUTOR_UNREACHABLE');
  }
  if (!ready && input.gasDeployed && input.gasAuthorized && !input.bridgeAuthAvailable) {
    blockers.push('BRIDGE_AUTH_UNAVAILABLE');
  }
  ...
  return {
    ...
    allowedStatus: ready ? 'running' : 'blocked'
  };
}
...
export function buildExecutionContract(input) {
  ...
  return {
    ...
    runtime_policy: 'preflight_before_implementation_or_success_claim',
    completion_gate: [
